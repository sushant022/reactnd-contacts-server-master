...
connectionsubject = mongoose.createConnection(config.dbUrl);
var PINCODE = require('./app/models/pincode');
...
app.get('/pincode/:id', (req, res) => {
	...
	console.log("type is" + typeof(req.params.id))
	var a= parseInt(req.params.id)

	PINCODE.find({pincode:a},{},function(err,pincode){
		if (err) 
			res.send(err);
		res.send(pincode);
	})
